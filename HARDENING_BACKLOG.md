...
**Widened by the 2026-09-10 sweep.** This entry assumes a deployment exists to
validate. None does.

```text
FACT                infra/ contains exactly one file, docker-compose.yml, a
                    local development stack: POSTGRES_PASSWORD: anci inline,
                    ports published to the host, no TLS, no restart policy, no
                    api healthcheck. There is no apps/web/Dockerfile — CI runs
                    npm run build and produces no artefact. No production
                    compose, no orchestration manifest, no infrastructure as
                    code, no secrets mechanism, no migrate-on-deploy step.
EXISTING AUTHORITY  this entry, for the routing question only.
CLASSIFICATION      known debt — the routing validation this entry asks for
                    cannot begin until there is something to route.
SEVERITY            🔴 unchanged. Evidence: the validation is a precondition
                    for cookie-backed auth being production-safe, and it is
                    blocked on work nobody has scoped.
BLOCKS              production.
```

Environment hygiene is clean and was checked: `dev.db` and
`apps/web/.env.local` are both gitignored and untracked, and `.env.local`
carries only `NEXT_PUBLIC_API_URL`.
...
**The audit this entry asks for was run on 2026-09-10.** `compare_metadata`
against the live migrated PostgreSQL returns **12 differences**. This entry
predicted "the same asymmetry may exist on other tables"; it does.

```text
FACT   five DB-side constraints exist in NO model, so an autogenerated
       migration would DROP them:
         ...
         the auth_sessions.token_hash uniqueness, expressed as a constraint
           in the DB and as a unique index in the model
       They enforce "one hour target per user/store/week" and "one pending
       request per requester per shift" — business invariants, not hygiene.

       two model indexes do NOT exist in the DB:
         ix_rota_recommendation_drafts_created_by_user_id
         ix_rota_recommendation_items_proposed_user_id

       rota_recommendation_drafts.status   DB TEXT   model String(32)
       auth_security_events.metadata_json  DB JSONB  model JSON   see H142
       auth_tokens.metadata_json           DB JSONB  model JSON   see H142
```

**Correction to this entry's Fix.** `conftest.py` does **not** call
`create_all`. **49 individual test files** do, each building its own SQLite
engine, and `conftest.py:14-20` rewrites every `sqlite:///` URL to in-memory
SQLite. Only four files use the real `DATABASE_URL`. The remediation is
roughly fifty times the size this entry implies, and it is a test-architecture
change rather than a fixture tweak.

```text
FACT                every constraint that lives only in a migration — the five
                    above, this entry's coverage_templates CHECKs, JSONB — is
                    absent from the schema 49 test files run against.
EXISTING AUTHORITY  this entry; H142 for metadata_json; CLAUDE.md's "Alembic
                    migrations only. No create_all."
CLASSIFICATION      known debt, understated. The drift and the test schema are
                    one defect: models are the test schema, so anything only a
                    migration knows is both untested and droppable.
SEVERITY            🟢 → 🟡. Evidence: this is no longer "may exist elsewhere"
                    but five named business invariants that no test exercises
                    and that autogenerate would remove.
BLOCKS              a phase. Fixing the drift without fixing the test schema
                    leaves the mechanism that produced it.
```

Q.5.3a-1's own test file is the counter-example and shows the target shape: it
provisions a throwaway PostgreSQL schema and runs `alembic upgrade head`.
...
**Confirmed live 2026-09-10.** Both columns appear in the 12-difference
`compare_metadata` result recorded in H110 — `auth_security_events.metadata_json`
and `auth_tokens.metadata_json`, DB `JSONB` against model `JSON`. Unchanged
since this entry was written; recorded so the audit's output and this entry do
not drift apart.
...
### H152 — The registration endpoint is the only auth route with no rate limit
...
```text
FACT                POST /api/v1/auth/register (auth.py:955) carries no
                    @limiter.limit decorator. Every other auth route does:
                    login, password-reset request and confirm, email
                    verification request and confirm, 2FA verify, disable,
                    recovery regeneration and step-up. It is unauthenticated
                    and each call creates a User, a Tenant and a TenantUser
                    row with role="owner".
EXISTING AUTHORITY  D001 records the registration contract, and public
                    onboarding is the product direction. H049 established the
                    rate-limiting baseline and named the routes it covered;
                    this one was not among them.
CLASSIFICATION      new defect — an omission against an existing convention,
                    not a product question.
SEVERITY            🟡. Evidence: no authentication bypass, no data exposure,
                    no cross-tenant read — rows are created, not disclosed.
                    The exposure is unbounded anonymous row creation by an
                    unauthenticated caller. It is 🟡 rather than 🔴 because
                    the worst outcome is resource exhaustion and signup spam,
                    and 🟡 rather than 🟢 because the endpoint is public and
                    the omission is inconsistent with every sibling route.
BLOCKS              first customer.
```

This is abuse protection, not a business-model question. Whether strangers may
create tenants is settled; how fast they may do so is not.

**Fix:** Apply a rate limit consistent with the other public auth routes.
Consider whether the limit belongs on IP alone or also on an identifier, given
H071 and H074 already track identifier-scoped limits for adjacent routes.

**Suggested phase:** With any auth or abuse-protection work.
...
FACT                is_revoked is read in exactly one dependency,
                    ...
                    get_current_tenant_id (deps.py:104),
                    require_tenant_member (deps.py:136), require_tenant_role
...
EXISTING AUTHORITY  none for this window. H057 (Done) covers revocation and
                    ...
                    H066 covers reuse detection, H067 all-sessions logout.
CLASSIFICATION      decision required.
SEVERITY            🔴. Evidence: this is the control password recovery
                    depends on. _revoke_active_admin_sessions_for_password_reset
                    exists to eject whoever compromised an account; the
                    ejection does not take effect on ordinary endpoints until
                    the access token expires.
BLOCKS              Q.5.3a-2, and first customer.
```

**What logout does do,** stated so this entry is not read as more than it is:
logout revokes the server-side session and blocks refresh reuse. Reuse
detection revokes the family. Disabled users are rejected. What survives is the
already-issued access JWT on ordinary endpoints, bounded by
`ACCESS_TOKEN_EXPIRE_MINUTES`.

The same mechanism carries a second case: a user demoted from `admin` to
`member` keeps access to every `require_tenant_member` endpoint for the same
window. D041's guard is at token issuance, so refresh is blocked, but the live
token is not re-checked.

**The question this entry needs answered:**

> When an `auth_session` is revoked, should an already-issued access token be
> invalid immediately, or is a bounded stale-access window accepted?

Both answers are defensible and they have different costs — immediate
invalidation puts a database read on every authenticated request; a bounded
window is the conventional short-lived-token trade-off, but it must be a
recorded position rather than an artefact of which dependency was written
first. **It is not the implementer's decision.**

**This gates Q.5.3a-2.** That phase ships the password-recovery UI on top of
this control. Settle the question before drafting it.

**Fix:** Adjudicate, then implement whichever answer is chosen — including
"accepted, with the window recorded", which is a valid outcome.

**Suggested phase:** Before Q.5.3a-2.
...
```text
FACT                H134 gave ENV and EMAIL_BACKEND validators that fail at
                    Settings construction. Nothing else received one:
                      JWT_SECRET_KEY = "dev-secret-change-me"  settings.py:22
                      BCRYPT_TEST_FAST = False                 settings.py:24
                      TOTP_ENCRYPTION_KEY = None               settings.py:35
                      CORS_ORIGINS = []                        settings.py:36
                      RATE_LIMIT_ENABLED = True                settings.py:37
                      LOG_LEVEL = "INFO"                       settings.py:20
                    CORS_ORIGINS is passed to CORSMiddleware with
                    allow_credentials=True (main.py:42-43) and nothing
                    rejects ["*"]. LOG_LEVEL reaches
                    getattr(logging, value.upper()), which can return a
                    non-integer attribute.
EXISTING AUTHORITY  H134 (Done) established the pattern for two settings.
                    H148 already tracks JWT_ALGORITHM as a seventh member of
                    this class. H075 covers TOTP key management but not
                    startup validation. README:265 documents JWT_SECRET_KEY as
                    required in production — documentation, not enforcement.
CLASSIFICATION      new defect, as a class.
SEVERITY            🔴, carried by JWT_SECRET_KEY alone. Evidence: a
                    production deploy that omits it starts successfully and
                    signs every access token with a secret committed to this
                    repository, which is total authentication compromise and
                    is silent. The remaining five are 🟡 or 🟢 individually;
                    they are grouped here because they share one fix and one
                    precedent.
BLOCKS              production.
```

Each of these is latent — every one needs a misconfiguration to fire. That is
...
### H155 — Interactive API documentation is public in every environment
...
```text
FACT                main.py:34 constructs FastAPI(title=settings.APP_NAME)
                    with no docs_url/redoc_url/openapi_url override, so /docs,
                    /redoc and /openapi.json serve the complete 89-route
                    schema unauthenticated in every environment. GET
                    /api/v1/health additionally returns settings.ENV
                    (health.py:13).
EXISTING AUTHORITY  none. H137 covers absent security response headers, which
                    is adjacent but different.
CLASSIFICATION      new defect.
SEVERITY            🟢. Evidence: no credential, no tenant data and no
                    authenticated capability is exposed — the schema describes
                    routes that still enforce their own guards. It is
                    reconnaissance value, not access. Recorded because
                    disabling it outside development is conventional and
                    costs one argument.
BLOCKS              production.
```

**Fix:** Gate the documentation routes on environment, deciding deliberately
which environments serve them.

**Suggested phase:** With H154 or H137.

---

### H156 — The `hot_food` router predates every RBAC convention its siblings follow

**Severity:** 🟡
**Status:** Open
**Area:** RBAC / tenant scoping

```text
FACT                three defects in one 68-line file:
                    1. GET /api/v1/hot-food/forecast (hot_food.py:20-21) has
                       no auth dependency of any kind.
                    2. The two demand-input routes guard with
                       get_current_user + get_current_tenant_id
                       (hot_food.py:30-31) and no role check. It is the only
                       admin-side router with none; every other uses
                       require_tenant_role.
                    3. create_hot_food_demand_input (hot_food.py:34) writes a
                       caller-supplied store_id with no check that the store
                       belongs to the caller's tenant.
EXISTING AUTHORITY  none for this router. D041 governs which roles are Admin
                    Portal access; D042 governs the tenant isolation gate.
CLASSIFICATION      new defect.
SEVERITY            🟡. Evidence: (1) is latent — forecast_hot_food is a stub
                    returning {"forecast": [], "model": "stub"}, so it
                    discloses nothing today and becomes live disclosure the
                    moment the service is implemented. (2) and (3) are live
                    but bounded: rows are tagged with the caller's tenant_id
                    and reads filter on it, so there is no cross-tenant read;
                    the unvalidated store_id creates a dangling reference, not
                    a leak. No credential or pay data is reachable here.
BLOCKS              first customer.
```

The forecast stub is what makes this easy to forget — the file looks inert.

**Fix:** Bring the router to the conventions its siblings follow: a role guard
consistent with D041, tenant-ownership validation on the written `store_id`,
and an auth dependency on the forecast route before the stub is replaced.

**Suggested phase:** With any hot-food work, or a small RBAC consistency pass.

---

### H157 — Every rate limit is asserted by tests that no green run executes

**Severity:** 🟡
**Status:** Open
**Area:** Test fidelity / abuse protection

```text
FACT                exactly six tests carry
                    @pytest.mark.skipif(not settings.RATE_LIMIT_ENABLED):
                      test_auth.py:159                    login
                      test_hot_food_demand_inputs.py:171  demand input create
                      test_phase_q5_1_totp_2fa.py:330     2FA verify
                      test_phase_q5_1_totp_2fa.py:784     2FA disable
                      test_phase_q5_1_totp_2fa.py:810     recovery regenerate
                      test_phase_q5_2a_step_up.py:334     2FA step-up
                    conftest.py:8 sets RATE_LIMIT_ENABLED=false before import,
                    and the CI backend job passes -e RATE_LIMIT_ENABLED=false
                    explicitly (ci.yml:32). These six are the "6 skipped" in
                    every recorded run, including the 602/0/6 and 656/0/6
                    baselines.
EXISTING AUTHORITY  none. H049 (Done) established the limits and H089
                    (Resolved) made the bootstrap deterministic by disabling
                    them in tests — which is why this exists.
CLASSIFICATION      new defect — a coverage gap, not a broken control.
SEVERITY            🟡. Evidence: the limits are wired and there is no
                    evidence any is broken. The exposure is that a regression
                    in any of them — login, password reset, email
                    verification, four 2FA routes, demand inputs — produces a
                    green suite and a green CI run. H152's missing limit is
                    the kind of gap this coverage would have caught.
BLOCKS              first customer confidence.
```

**Fix:** Give the rate-limit assertions a run in which they execute. A separate
CI job or pytest invocation with `RATE_LIMIT_ENABLED=true` would satisfy it
without reintroducing the cross-test counter accumulation H089 resolved, since
...
## Root-cause grouping — 2026-09-10 repository sweep

Not backlog entries. Recorded because several entries above share one cause,
and closing them together is cheaper and safer than closing them separately.
This records the grouping only; it creates no phase and schedules nothing.

```text
A   authorization is decided once, at token issuance, and never revisited
    H153, H069
    One change — validating sid against auth_sessions in the shared
    dependencies, and dropping the refresh token from the response body —
    addresses logout, password-reset ejection, reuse detection and role
    demotion together. Today each wears a different number.

B   H134's fail-closed pattern was applied to two settings and stopped
    H154, H148, H155
    ENV and EMAIL_BACKEND validate at construction; JWT_SECRET_KEY,
    JWT_ALGORITHM, TOTP_ENCRYPTION_KEY, BCRYPT_TEST_FAST, RATE_LIMIT_ENABLED,
    CORS_ORIGINS, LOG_LEVEL and the docs routes do not. One settings phase.

C   the declarative models are not the schema, and the tests use the models
    H110, H142, H126
    The drift and the test schema are one defect. Repairing the drift without
    changing how tests build schema leaves the mechanism that produced it.

D   controls exist but nothing proves they work
    H157, H110
    Rate limits are skipped; migration-only constraints are absent from the
    test schema. Both yield green runs that prove less than the record claims
    — the failure mode H147 was opened for, one layer down.

E   hot_food is an unguarded island
    H156
    Three defects in one file, all from predating the conventions every
    sibling router follows. Cheapest to close; likeliest to be forgotten,
    because the forecast stub makes the file look inert.

F   standalone
    H152   abuse protection against a settled product direction
    H068   nothing is deployable; greenfield, not repair
```
