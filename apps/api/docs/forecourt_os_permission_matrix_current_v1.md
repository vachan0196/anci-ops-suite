...
| Area | Endpoint(s) | CURRENT-TRUTH | Field / Data Notes | Evidence |
|---|---|---|---|---|
| Staff list | `GET /api/v1/staff` | Owner/admin endpoint. Tenant-scoped. Owner receives full staff profile data. Admin receives safe staff projection with pay/RTW fields omitted. | Owner responses include `hourly_rate`, `pay_type`, `rtw_status`. Non-owner responses omit those keys entirely, not `null`. Directory remains trimmed. | Staff.2, `StaffProfileSafeOut`, `test_staff2_pay_rtw_read_hardening.py`. |
| Staff create | `POST /api/v1/staff` | Owner/admin endpoint. Tenant-scoped. Owner can write pay/RTW fields. Admin can create basic staff profiles but cannot write non-null pay/RTW fields. | Non-owner non-null `hourly_rate`, `pay_type`, or `rtw_status` is rejected with 403. Non-owner explicit `null` values are stripped and treated as not setting. Employee password is hashed and not returned. | Staff.2b, `test_staff2b_pay_rtw_write_hardening.py`. |
...
| Staff detail | `GET /api/v1/staff/{staff_id}` | Active tenant member dependency. Owner can read full staff profile in active tenant. Admin can read safe staff profile projection. Non-admin member can read only own safe profile. Cross-tenant returns 404. | Owner response includes `hourly_rate`, `pay_type`, `rtw_status`. Non-owner responses omit pay/RTW keys entirely. | Staff.2, T.2 matrix update, `test_staff2_pay_rtw_read_hardening.py`. |
...
| Staff pay/RTW | Pay and right-to-work/compliance fields should be owner-only for MVP unless explicitly changed. Current Staff.2 / Staff.2b implementation now matches this for `hourly_rate`, `pay_type`, and `rtw_status`. |
...
|---|---|---|---|---|---|
| Staff pay/RTW admin read | RESOLVED in Staff.2: Owner receives pay/RTW fields; Admin/non-owner staff read responses omit `hourly_rate`, `pay_type`, and `rtw_status`. | Pay/RTW owner-only for MVP unless explicitly granted. | Current read exposure fixed for implemented pay/RTW fields. | resolved-current-truth | Staff.2 completed; keep future owner-only UI and audit/step-up as H083/H060/H073 follow-up. |
| Staff pay/RTW admin write | RESOLVED in Staff.2b: Owner can write pay/RTW; Admin/non-owner non-null writes are rejected with 403; explicit `null` values are stripped and cannot clear Owner-set values. | Owner-only or explicit sensitive permission; likely step-up for future owner-only pay/RTW UI. | Current write exposure fixed for implemented pay/RTW fields. | resolved-current-truth | Staff.2b completed; future owner-only pay/RTW UI tracked by H083. |
| Staff self full response | RESOLVED in Staff.2 for pay/RTW: member-accessible admin-style own-profile reads use safe projection and omit pay/RTW keys. | Employee/member self APIs should return employee-safe fields only, with any pay display intentionally designed. | Current pay/RTW staff self leakage fixed for implemented fields. | resolved-current-truth | Continue staff identity decoupling / employee API consolidation under H080 if needed. |
...
| Staff self pay/RTW legacy surface | RESOLVED for implemented pay/RTW fields in Staff.2 | Non-owner/member-accessible staff self reads now use safe projection and omit pay/RTW keys. Staff identity decoupling remains separate. | resolved-current-truth |
