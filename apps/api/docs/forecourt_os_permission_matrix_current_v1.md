...
> CURRENT-TRUTH is authoritative only as of commit `5b79955`.
...
| Store update | `PATCH /api/v1/stores/{store_id}` | Owner/admin. Cross-tenant store ID returns 404. | Can update `is_active` without the sensitive deactivation gate. This differs from dedicated deactivate endpoint. | `stores.py`, `StoreUpdate`, T.0 tests. |
...
| Store update `is_active` | `PATCH /stores/{id}` can set `is_active`; dedicated deactivate endpoint is owner + 2FA step-up. | Destructive/deactivation semantics should consistently use sensitive-action gate. | Possible bypass of deactivation gate if frontend/client uses PATCH. | fix-before-onboarding | H081/H060 follow-up. |
