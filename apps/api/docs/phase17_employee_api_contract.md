...
- Phase Q.5.1/Q.5.1b implement admin-side TOTP enrolment, login challenge verification, recovery-code use, disable 2FA, and recovery-code regeneration only. Phase Q.5.2a adds admin-side session-bound step-up for sensitive actions and applies it to store deactivation. Employee auth and employee portal APIs are unchanged; employee 2FA remains future work.
